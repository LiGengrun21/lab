<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="showlabList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="房间设备数" align="center" prop="equipmentNum" />
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listShowlabEquipment(this.queryParams).then(response => {
        this.showlabList = response.rows;
        for (var i = 0; i < this.showlabList.length; i++) {
          for (var j = 0,k = 0; j < this.showlabList[i].zjutEquipmentList.length; j++) {
            if(this.showlabList[i].zjutEquipmentList[j].equipmentId != null){
              k++;
            }
          }
          this.showlabList[i].equipmentNum = k;
          console.log(this.showlabList[i].equipmentNum);
        }
        this.total = this.showlabList.length;
        this.loading = false;
