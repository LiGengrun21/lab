<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--<el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['equipment:show:export']"
        >导出</el-button>
      </el-col>-->
    ...
    <el-table v-loading="loading" :data="showList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="设备名称" align="center" prop="equipmentName"  >
        <template slot-scope="scope">
          <el-tag>{{scope.row.equipmentName}}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="设备状态" align="center" prop="equipmentStatus" :formatter="statusFormat" />
      <el-table-column label="是否可预约" align="center" prop="equipmentSubscribeType"
      :filters="[{text: '可预约', value:'0'}, {text: '不可预约', value: '1'}]"
      :filter-method="filterHandler">
            <template slot-scope="scope">
                <el-button type="primary"  size="mini" v-show="scope.row.equipmentSubscribeType==='0'"  @click="openl(scope.row)"  icon="el-icon-s-promotion"> 可预约</el-button>
                <el-button type="warning" size="mini" v-show="scope.row.equipmentSubscribeType==='1'"  @click="openindex=true"  icon="el-icon-remov" style="margin-left:-3px"> 不可预约</el-button>
            </template>
      </el-table-column>
    ...
    />
     <!--设备收费设置详情及其修改-->
    <el-dialog :visible.sync="openindex"  append-to-body width="700">
        <APPointer :equmentId="equiment_id" :equimentName="equimentName"></APPointer>
        <el-button @click="APPointerCan" type="primary" class="close">关闭</el-button>
    </el-dialog>
    <!-- 添加或修改设备对话框 -->
    ...
    <!--<el-dialog :title="title" :visible.sync="open3" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="设备名称" prop="equipmentName">
          <el-input v-model="form.equipmentName" placeholder="请输入设备id" :disabled="true" />
        </el-form-item>
      ...
      <el-upload
        ...
        :on-success="handleDestoryFileSuccess"
        :auto-upload="false">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleAddDestory">报 废</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>-->

    <!-- 添加或修改设备维保对话框 -->
    <el-dialog :title="title" :visible.sync="open4" :rules="rules" width="500px"  append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="设备名称" prop="equipmentName">
          <el-input v-model="form.equipmentName" placeholder="请输入设备id" :disabled="true" />
        </el-form-item>
        <el-form-item label="维保原因" prop="fixReason">
          <el-input v-model="form.fixReason" placeholder="请输入维保原因" />
        </el-form-item>
        <el-form-item label="维保开始日期" prop="fixStartTime">
          <el-date-picker clearable size="small"
                          v-model="form.fixStartTime"
                          type="date"
                          value-format="yyyy-MM-dd"
                          placeholder="选择开始日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="维保结束日期" prop="fixEndTime">
          <el-date-picker clearable size="small"
                          v-model="form.fixEndTime"
                          type="date"
                          value-format="yyyy-MM-dd"
                          placeholder="选择结束日期">
          </el-date-picker>
      ...
      <el-upload
        ref="upload"
        :limit="5"
        accept=".jpg, .png, .pdf, .docx, .xlsx, .doc"
        :action="upload.url"
        :headers="upload.headers"
        :file-list="upload.fileList"
        :on-progress="handleFileUploadProgress"
        :on-success="handleFixFileSuccess"
        :auto-upload="false">
        <el-button slot="trigger" size="small" type="primary">选取文件</el-button>
        <el-button style="margin-left: 10px;" size="small" type="success" :loading="upload.isUploading" @click="submitUpload">确认上传</el-button>
        <div slot="tip" class="el-upload__tip">只能上传jpg/png/pdf/word文件</div>
      </el-upload>

      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleAddFix">维 保</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>
...
<script>
import {listShow,getShow,delShow,addShow,updateShow,exportShow,listShowSubscribe,getDeviceStatus, switchOn, switchOff} from '@/api/equipment/show'
import { listDestory, getDestory, delDestory, addDestory, updateDestory, exportDestory } from "@/api/equipment/destory";
import {createDevicelog} from "@/api/equipment/devicelog"
...
import  APPointer   from "./components/appointment"
export default {
  name: "Show",
  components: {
    "APPointer":APPointer
  ...
  data() {
    return {
      ...
      rules: {
        ...
        fixStartTime: [
          { required: true, message: "报修开始时间不能为空", trigger: "blur" }
        ],
        fixEndTime: [
          { required: true, message: "报修结束时间不能为空", trigger: "blur" }
        ]
      },
      selectEquirment:0
    };
  ...
  methods: {
    handleCreateDeviceLog(row){
      const equipmentId = row.equipmentId || this.ids;
      console.log(equipmentId);
      createDevicelog(equipmentId).then(responce =>{
        console.log(responce);
      });
    },
    //打开预约设置页面
    openl(row){
      this.equimentName=row.equimentName
      this.equiment_id=row.equipmentId
      this.openindex=true
    },
    //点击关闭现在的设备费用详情
    APPointerCan(){
