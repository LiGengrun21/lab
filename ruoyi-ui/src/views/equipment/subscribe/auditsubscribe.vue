...
import {
  ...
  updateSubscribe,
  exportSubscribe,
  listAuditSubscribe, listMySubscribe
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAuditSubscribe(this.queryParams).then(response => {
    ...
    handleFail(row) {
      ...
      getSubscribe(equipmentSubscribeId).then(response => {
        this.form = response.data;
        console.log(this.form);
        this.form.subscribeStatus = 3;
        updateSubscribe(this.form).then(response => {
          this.msgSuccess("拒绝预约成功");
          this.getList();
        });
      });
    },
