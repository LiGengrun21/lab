    ...
    <el-table v-loading="loading" :data="subscribeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="预约状态" align="center" prop="subscribeStatus"  >
        <template slot-scope="scope">
          ...
          <span style="color:#DAA520" v-if="scope.row.subscribeStatus== 1">预约完成</span>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            icon="el-icon-delete"
          >
          <span v-if="!TimeJudge(scope.row.subscribeDate+' '+scope.row.subscribeEndTime)" @click="delectFor(scope.row)">取消预约</span>
          <span v-if="TimeJudge(scope.row.subscribeDate+' '+scope.row.subscribeEndTime)" @click="handleDelete(scope.row)">删除记录</span>
          </el-button>
...
export default {
  ...
  created() {
    ...
    this.getList();
    this.getequip();
    this.TimeJudge("2021-09-11"+ " 11:00:00")
  },
  methods: {
    /**取消预约 */
    delectFor(row){
      this.$confirm('是否确认取消设备预约编号为"' + row.equipmentSubscribeId + '"的预约?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(()=>{
        cancelSubscribe({equipmentSubscribeId:row.equipmentSubscribeId}).then(res=>{
    ...
    getList() {
      this.loading = true;
      listMySubscribe(this.queryParams).then(response => {
